...
import time
from common.utils import square_distance
...
def collate_fn_descriptor_lrf(list_data, config, neighborhood_limits):
    batched_points_list = []
    batched_features_list = []
    batched_lengths_list = []
    batched_lrfs_list = []
    ...
    for ind, (src_pcd, tgt_pcd, src_feats, tgt_feats, rot, trans, matching_inds, src_pcd_raw, tgt_pcd_raw, sample, src_lrfs, tgt_lrfs) in enumerate(list_data):
        batched_points_list.append(src_pcd)
        batched_points_list.append(tgt_pcd)
        batched_features_list.append(src_feats)
        batched_features_list.append(tgt_feats)
        ...
        batched_lengths_list.append(len(tgt_pcd))
        # lrf
        batched_lrfs_list.append(src_lrfs)
        batched_lrfs_list.append(tgt_lrfs)

    batched_features = torch.from_numpy(np.concatenate(batched_features_list, axis=0))
    batched_points = torch.from_numpy(np.concatenate(batched_points_list, axis=0))
    batched_lengths = torch.from_numpy(np.array(batched_lengths_list)).int()
    batched_lrfs = torch.cat(batched_lrfs_list, dim=0)
    ...
    for block_i, block in enumerate(config.architecture):
        ...
        input_upsamples += [up_i.long()]
        input_batches_len += [batched_lengths]
        input_lrfs += [batched_lrfs.float()]

        # print(pool_p.shape, batched_points.shape)
        p_idx = None
        if pool_p.shape[0] != 0:
            p_idx = square_distance(pool_p.unsqueeze(0).float(), batched_points.unsqueeze(0).float())[0].min(1)[1]
        if p_idx is not None:
            batched_lrfs = batched_lrfs[p_idx]
...
def get_dataloader_lrf(dataset, batch_size=1, num_workers=4, shuffle=True, neighborhood_limits=None):
    if neighborhood_limits is None:
        neighborhood_limits = calibrate_neighbors(dataset, dataset.config, collate_fn=collate_fn_descriptor_lrf)
    print("neighborhood:", neighborhood_limits)
    dataloader = torch.utils.data.DataLoader(
        dataset,
        batch_size=batch_size,
        ...
        num_workers=num_workers,
        # https://discuss.pytorch.org/t/supplying-arguments-to-collate-fn/25754/4
        collate_fn=partial(collate_fn_descriptor_lrf, config=dataset.config, neighborhood_limits=neighborhood_limits),
        drop_last=False
    )
    return dataloader, neighborhood_limits
