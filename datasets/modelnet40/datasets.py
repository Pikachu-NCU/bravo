...
import datasets.modelnet40.transforms as Transforms
import common.math.se3 as se3

_logger = logging.getLogger()


def get_train_datasets(train_categoryfile, val_categoryfile, noise_type, rot_mag, trans_mag, num_points, partial, dataset_path):
    train_categories, val_categories = None, None
    if train_categoryfile:
    ...
    train_transforms, val_transforms = get_transforms(noise_type, rot_mag, trans_mag, num_points, partial)
    ...
    train_data = ModelNetHdf(dataset_path, subset='train', categories=train_categories, transform=train_transforms)
    val_data = ModelNetHdf(dataset_path, subset='test', categories=val_categories, transform=val_transforms)

    return train_data, val_data


def get_test_datasets(test_category_file, noise_type, rot_mag, trans_mag, num_points, partial, dataset_path):
    test_categories = None
    if test_category_file:
        test_categories = [line.rstrip('\n') for line in open(test_category_file)]
        test_categories.sort()

    _, test_transforms = get_transforms(noise_type, rot_mag, trans_mag, num_points, partial)
    _logger.info('Test transforms: {}'.format(', '.join([type(t).__name__ for t in test_transforms])))
    test_transforms = torchvision.transforms.Compose(test_transforms)

    test_data = ModelNetHdf(dataset_path, subset='test', categories=test_categories, transform=test_transforms)
