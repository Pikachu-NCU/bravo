...
def compute(pc, x_theta_space=1/6, z_theta_space=1/6):
    ...
    x_index, z_index = (x_theta / (x_theta_space*math.pi)).int(), (z_theta / (z_theta_space*math.pi)).int()
    # 确保范围不因精度问题而越界，正常情况下应该是不执行的
    x_index[x_index > (x_theta_kind-1)] = x_theta_kind-1
    z_index[z_index > (z_theta_kind-1)] = z_theta_kind-1
