...
class LRF:
    ...
    def get_lrf(self, pt):
        ...
        if len(patch_idx) < 5:
            _, patch_idx, _ = self.pcd_tree.search_knn_vector_3d(pt, 30)
            ptnn = np.asarray(self.pcd.points)[patch_idx[1:], :].T
            ptnn_cov = 1 / len(ptnn) * np.dot((ptnn - pt[:, np.newaxis]), (ptnn - pt[:, np.newaxis]).T)
        ...
        beta = np.ones_like(alpha.shape)
        # e.q. 5
        # xp = np.dot(v, (alpha * beta)[:, np.newaxis])
        # xp = F.normalize(torch.from_numpy(xp), dim=0).numpy()
        # xp = F.normalize(torch.from_numpy(v[:, 2]), dim=0).numpy()
        # print(xp)
        # if np.linalg.norm(np.dot(v, (alpha * beta)[:, np.newaxis])) == 0:
        #     return np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        if np.linalg.norm(np.dot(v, (alpha * beta)[:, np.newaxis])) == 0:
            print(ptnn.shape, np.dot(v, (alpha * beta)[:, np.newaxis]), zp, v.T)
            patch_pc = o3d.geometry.PointCloud()
            patch_pc.points = o3d.utility.Vector3dVector(ptnn.T)
            o3d.visualization.draw_geometries([self.pcd])
            o3d.visualization.draw_geometries([patch_pc])
