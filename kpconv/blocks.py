...
class KPConv(nn.Module):
    ...
    def forward(self, q_pts, s_pts, neighb_inds, x, q_lrf=None):
        ...
        if q_lrf is not None:
            # patch lrf
            neighbors = torch.matmul(q_lrf, neighbors.permute([0, 2, 1])).permute([0, 2, 1])
...
class SimpleBlock(nn.Module):
    ...
    def forward(self, x, batch):
        ...
        if 'strided' in self.block_name:
            ...
            if batch.get('lrfs') is not None:
                q_lrf = batch['lrfs'][self.layer_ind + 1]
        else:
            q_pts = batch['points'][self.layer_ind]
            s_pts = batch['points'][self.layer_ind]
            neighb_inds = batch['neighbors'][self.layer_ind]
            if batch.get('lrfs') is not None:
                q_lrf = batch['lrfs'][self.layer_ind]

        x = self.KPConv(q_pts, s_pts, neighb_inds, x, q_lrf)
